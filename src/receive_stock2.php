    ...
    if (empty($items) || empty($locId)) {
        ...
        try {
            foreach ($items as $item) {
                ...
                // B. Update Product Cost (Weighted Average or Last Cost? using Last Cost for simplicity)
                if ($cost > 0) {
...
// Determine Default Location (Session > First Available)
$currentLocId = $_SESSION['pos_location_id'] ?? ($locations[0]['id'] ?? 0);
?>
