...
// --- 1. SERVICE MANAGEMENT (NEW: Fixes Service Registration) ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_service'])) {
    ...
    try {
        ...
        $pdo->prepare("INSERT INTO products (name, sku, price, cost_price, type, is_open_price, is_active) VALUES (?, '', ?, 0, 'service', ?, 1)")
            ->execute([$name, $price, $isOpen]);
        $_SESSION['swal_type'] = 'success';
        $_SESSION['swal_msg'] = "Service '$name' added.";
...
// --- 3. ADD TO TAB (UPDATED: Splits Items Individually) ---
if (isset($_POST['add_to_tab_action']) && !empty($_SESSION['cart'])) {
    ...
    try {
        ...
        foreach ($_SESSION['cart'] as $pid => $item) {
            ...
            // EXPLODE ITEM: Loop through quantity and insert 1 by 1
            for ($k = 0; $k < $qty; $k++) {
                ...
                // Deduct Inventory for this single unit
                if (($item['type'] ?? 'item') !== 'service') {
                    ...
                    $pdo->prepare("INSERT INTO inventory_logs (product_id, location_id, user_id, change_qty, after_qty, action_type, reference_id, created_at) VALUES (?, ?, ?, -1, ?, 'sale', ?, NOW())")
                        ->execute([$pid, $locId, $userId, $nq, $saleId]);
                }
            }
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($activeShiftId) {
        ...
        if (isset($_POST['toggle_fulfillment'])) { $pid = $_POST['product_id']; if(isset($_SESSION['cart'][$pid])) { $_SESSION['cart'][$pid]['fulfillment'] = ($_SESSION['cart'][$pid]['fulfillment'] === 'collected') ? 'uncollected' : 'collected'; } header("Location: index.php?page=pos"); exit; }
        ...
        if (isset($_POST['recall_tab'])) {
            ...
            foreach($items as $i) { 
                ...
                if(isset($_SESSION['cart'][$pid])) { $_SESSION['cart'][$pid]['qty'] += $i['quantity']; } 
                    ...
                    $_SESSION['cart'][$pid] = ['name'=>$n['name'], 'price'=>$i['price_at_sale'], 'qty'=>$i['quantity'], 'type'=>$n['type'], 'fulfillment'=>$i['fulfillment_status']]; 
                }
            }
            header("Location: index.php?page=pos"); exit;
        }
        if (isset($_POST['checkout']) && !empty($_SESSION['cart'])) {
            ...
            try {
                ...
                foreach ($_SESSION['cart'] as $pid => $item) {
                    $fulfill = $item['fulfillment'] ?? 'collected';
                    // Use Explode Logic Here? No, standard checkout groups items. Only Tab splits them.
