...
<body>
    ...
    <div class="modal fade" id="floorplanModal" tabindex="-1">
        <div class="modal-dialog modal-xl modal-dialog-scrollable"><div class="modal-content bg-light"><div class="modal-header bg-dark text-white border-warning border-bottom border-3"><h5 class="modal-title fw-bold"><i class="bi bi-grid-3x3-gap-fill me-2 text-warning"></i> Table Floorplan</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div><div class="modal-body p-4">
            <?php if(empty($restaurantTables)): ?><div class="text-center text-muted my-5"><i class="bi bi-info-circle display-4"></i><p class="mt-3">No tables have been configured for this location yet.</p></div><?php else: ?>
                <?php foreach($restaurantTables as $zoneName => $tables): ?><h5 class="fw-bold text-muted border-bottom pb-2 mb-3 mt-4"><?= htmlspecialchars($zoneName) ?></h5><div class="row g-3">
                    <?php foreach($tables as $table): $activeTab = null; foreach($openTabs as $t) { if ($t['table_id'] == $table['id']) { $activeTab = $t; break; } } $isOccupied = ($activeTab !== null); ?>
                    <div class="col-6 col-md-4 col-lg-3">
                        <?php if($isOccupied): ?>
                            <div class="table-box table-occupied" onclick="switchModal('floorplanModal', 'tabsModal', () => showTabDetails(<?= $activeTab['id'] ?>))">
                                <span class="table-capacity"><i class="bi bi-people-fill"></i> <?= $table['capacity'] ?></span><h5 class="fw-bold mb-1"><?= htmlspecialchars($table['table_name']) ?></h5><div class="small fw-bold">ZMW <?= number_format($activeTab['final_total'], 2) ?></div><div class="badge bg-danger mt-2">OCCUPIED</div>
                            </div>
                        <?php else: ?>
    ...
    <div class="modal fade" id="tabsModal" tabindex="-1">
        <div class="modal-dialog modal-xl"><div class="modal-content h-100"><div class="modal-header bg-dark text-white border-info border-bottom border-3"><h5 class="modal-title fw-bold"><i class="bi bi-receipt me-2 text-info"></i> Active Tabs</h5><button type="button" class="btn btn-sm btn-outline-info ms-auto me-3 fw-bold" onclick="switchModal('tabsModal', 'floorplanModal')"><i class="bi bi-grid-3x3-gap-fill"></i> View Tables</button><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div><div class="modal-body"><div class="row h-100"><div class="col-4 border-end overflow-auto"><div class="list-group">
            <?php foreach($openTabs as $t): $isPaid = $t['payment_status'] === 'paid'; $bg = $isPaid ? 'bg-success-subtle' : ''; $badge = $isPaid ? '<span class="badge bg-success">PAID</span>' : ''; ?>
            <button class="list-group-item list-group-item-action <?= $bg ?>" onclick="showTabDetails(<?= $t['id'] ?>)"><div class="d-flex justify-content-between"><strong><?= htmlspecialchars($t['customer_name']) ?></strong> <?= $badge ?></div><div class="small text-muted">ZMW <?= number_format($t['final_total'],2) ?></div></button>
            <?php endforeach; ?>
        </div></div><div class="col-8 p-3" id="tabDetailContainer"><p class="text-center text-muted mt-5">Select a tab from the list to view details.</p></div></div></div></div></div>
    </div>
                    ...
                    <td class="text-end">
                        ...
                        <?php if($i['fulfillment_status'] == 'uncollected'): ?>
                            <?php if(in_array(strtolower($i['cat_type'] ?? ''), ['food', 'meal']) && defined('LICENSE_TIER') && in_array(LICENSE_TIER, ['pro', 'hospitality'])): ?>
                                <?php if($i['status'] === 'ready'): ?>
                                    <span class="badge bg-warning text-dark border shadow-sm px-2 py-1" style="cursor:pointer;" onclick="switchModal('tabsModal', '', showPickupModal)"><i class="bi bi-bag-check"></i> GO TO PICKUP</span>
                                <?php else: ?>
                                    <span class="badge bg-secondary text-white border shadow-sm px-2 py-1" style="cursor:pointer;" onclick="Swal.fire({icon: 'info', title: 'Still Cooking', text: 'This item is still in preparation. You will be notified when it is ready for pickup!', timer: 2500, showConfirmButton: false})"><i class="bi bi-hourglass-split"></i> PREPARING</span>
                                <?php endif; ?>
                            <?php else: ?>
                                <span class="badge badge-uncollected p-2" style="cursor:pointer;" onclick="markCollected(<?= $i['id'] ?>, <?= $tid ?>)">MARK COLLECTED</span>
                            <?php endif; ?>
    ...
    <div class="modal fade" id="expenseModal" tabindex="-1"><div class="modal-dialog modal-dialog-centered"><div class="modal-content border-0 shadow-lg border-top border-warning border-4"><div class="modal-header bg-light"><h5 class="modal-title fw-bold text-dark"><i class="bi bi-cash-stack text-warning"></i> Log Petty Cash / Payout</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><form method="POST"><div class="modal-body p-4"><input type="hidden" name="log_expense" value="1"><div class="alert alert-warning small border-warning"><strong><i class="bi bi-info-circle"></i> Note:</strong> This will instantly deduct cash from your Expected Drawer Total.</div><div class="mb-3"><label class="form-label small fw-bold text-muted">Amount Taken (ZMW)</label><div class="input-group input-group-lg"><span class="input-group-text bg-white fw-bold border-end-0">ZMW</span><input type="number" step="0.01" name="expense_amount" class="form-control fw-bold border-start-0 text-danger" required placeholder="0.00"></div></div><div class="mb-4"><label class="form-label small fw-bold text-muted">Reason / Paid To</label><input type="text" name="expense_reason" class="form-control" required placeholder="e.g. Paid delivery driver, bought ice..."></div><div class="p-3 bg-dark rounded shadow-sm border border-secondary"><label class="form-label small fw-bold text-warning mb-3 d-block border-bottom border-secondary pb-2"><i class="bi bi-shield-lock-fill"></i> MANAGER AUTHORIZATION REQUIRED</label><input type="text" name="mgr_username" class="form-control mb-2" required placeholder="Manager Username"><input type="password" name="mgr_password" class="form-control" required placeholder="Manager Password"></div></div><div class="modal-footer bg-light border-0"><button type="button" class="btn btn-secondary fw-bold" data-bs-dismiss="modal">Cancel</button><button type="submit" class="btn btn-warning fw-bold shadow-sm px-4">Authorize Payout</button></div></form></div></div></div>
...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() { 
            ...
            let pickupMod = document.getElementById('pickupModal');
