...
<body>
    ...
    <div class="workspace" style="<?= (!$activeShiftId) ? 'filter: blur(5px); pointer-events: none;' : '' ?>">
        ...
        <div class="cart-panel" id="cartPanel">
            ...
            <div class="cart-items">
                <?php if (empty($_SESSION['cart'])): ?><div class="text-center mt-5 text-muted"><i class="bi bi-cart-x display-1 opacity-25"></i><p class="mt-3 fw-bold">Cart is empty</p></div><?php else: ?>
    ...
    <div class="modal fade" id="serviceManagerModal" tabindex="-1"><div class="modal-dialog modal-lg"><div class="modal-content"><div class="modal-header bg-dark text-white"><h5 class="modal-title"><i class="bi bi-stars text-warning"></i> Manage Services</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div><div class="modal-body">
        <form method="POST" class="row g-2 mb-4 bg-light p-3 rounded border">
            ...
            <div class="col-md-5"><input type="text" name="name" class="form-control" placeholder="Service Name (e.g. Delivery)" required></div>
            <div class="col-md-3"><input type="number" step="0.01" name="price" class="form-control" placeholder="Default Price" required></div>
            <div class="col-md-2 d-flex align-items-center"><div class="form-check"><input class="form-check-input" type="checkbox" name="is_open_price" value="1" id="isOpen"><label class="form-check-label small" for="isOpen">Adjustable?</label></div></div>
            <div class="col-md-2"><button class="btn btn-success w-100 fw-bold">Add New</button></div>
        </form>
        <div class="table-responsive"><table class="table table-hover align-middle"><thead class="table-secondary"><tr><th>Name</th><th>Default Price</th><th>Type</th><th>Action</th></tr></thead><tbody><?php foreach($services as $s): ?><tr><td class="fw-bold"><?= htmlspecialchars($s['name']) ?></td><td><?= $s['is_open_price'] ? '<span class="badge bg-info text-dark">Adjustable</span>' : 'ZMW '.number_format($s['price'], 2) ?></td><td><span class="badge bg-warning text-dark">Service</span></td><td><form method="POST" onsubmit="return confirm('Delete service?');"><input type="hidden" name="delete_service" value="1"><input type="hidden" name="service_id" value="<?= $s['id'] ?>"><button class="btn btn-sm btn-outline-danger"><i class="bi bi-trash"></i></button></form></td></tr><?php endforeach; ?></tbody></table></div>
    </div></div></div></div>
    ...
    <script>
        ...
        function switchTab(tab, btn) {
            document.querySelectorAll('.cat-pill').forEach(e=>e.classList.remove('active')); btn.classList.add('active');
            if (tab === 'services') { document.getElementById('items-grid').style.display = 'none'; document.getElementById('services-grid').style.display = 'flex'; }
            else { document.getElementById('items-grid').style.display = 'flex'; document.getElementById('services-grid').style.display = 'none'; }
        }

        function addService(id, name, price, isOpen) {
            if (isOpen) {
                document.getElementById('op_pid').value = id;
                document.getElementById('op_name').innerText = name;
                new bootstrap.Modal(document.getElementById('openPriceModal')).show();
            } else {
                // Submit Form programmatically
                let f = document.createElement('form'); f.method = 'POST'; f.innerHTML = `<input type="hidden" name="add_item" value="1"><input type="hidden" name="product_id" value="${id}">`;
