<div class="d-flex flex-column gap-4">
    <div class="d-flex justify-content-between align-items-end border-bottom pb-3" style="border-color: #e0e0e0;">
        ...
        <div class="d-flex gap-2">
            ...
            <a href="index.php?page=pos" class="btn btn-theme-orange text-white shadow fw-bold px-4 d-flex align-items-center"><i class="bi bi-cart4 me-2"></i> OPEN POS</a>
        </div>
    </div>
        ...
        <div class="col-6 col-md-3">
            <div class="card shadow-sm border-0 h-100 position-relative overflow-hidden">
                <div class="card-body">
                    ...
                    <h4 class="fw-bold text-brown text-truncate mb-0" title="<?= htmlspecialchars($myStats['top_item'] ?? '-') ?>"><?= htmlspecialchars($myStats['top_item'] ?? '-') ?></h4>
    ...
    <div class="row g-4">
        ...
        <div class="col-12">
            <div class="alert alert-danger shadow-sm border-0 d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center"><i class="bi bi-exclamation-octagon-fill display-6 me-3"></i><div><h5 class="alert-heading fw-bold m-0">Critical Stock Warning</h5><small><?= count($lowStockItems) ?> items are below minimum levels. Auto-requisitions have been generated.</small></div></div>
        ...
        <div class="col-md-6">
            <div class="card shadow-sm border-0 h-100">
                <div class="card-header bg-white border-bottom-0 pt-3 pb-0"><h6 class="fw-bold text-uppercase text-muted"><i class="bi bi-wallet2 text-warning me-2"></i> Unpaid Tabs</h6></div>
                <div class="card-body d-flex justify-content-between align-items-center">
                    <div><h2 class="fw-bold text-dark mb-0">ZMW <?= number_format($pendingTabs['total'], 2) ?></h2><span class="badge bg-warning text-dark mt-2"><?= $pendingTabs['count'] ?> Open Orders</span></div>
        ...
        <div class="col-md-6">
            <div class="card shadow-sm border-0 h-100">
                <div class="card-header bg-white border-bottom-0 pt-3 pb-0"><h6 class="fw-bold text-uppercase text-muted"><i class="bi bi-box-seam text-primary me-2"></i> Stock Transfers</h6></div>
                <div class="card-body d-flex justify-content-between align-items-center">
                    <div><h2 class="fw-bold text-dark mb-0"><?= $pendingReqs ?></h2><span class="badge bg-primary mt-2">Pending Movement</span></div>
    ...
    <div class="row g-4">
        <div class="col-lg-8">
            ...
            <div class="card shadow-sm border-0 h-100">
                <div class="card-header bg-white border-0 py-3 d-flex justify-content-between align-items-center"><h6 class="fw-bold m-0"><i class="bi bi-people-fill text-brown me-2"></i> Active Staff Monitor</h6><span class="badge bg-success bg-opacity-10 text-success border border-success"><?= count($activeStaff) ?> Online</span></div>
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="ps-3 fw-bold text-dark"><div class="d-flex align-items-center"><div class="rounded-circle bg-success me-2" style="width:8px; height:8px;"></div><?= htmlspecialchars($staff['username']) ?></div></td>
                                ...
                                <td class="text-end pe-3"><button class="btn btn-sm btn-link text-decoration-none text-warning fw-bold" onclick="viewShiftDetails(<?= $staff['shift_id'] ?>)">VIEW LIVE</button></td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
            ...
            <?php else: ?>
                <div class="card shadow-sm border-0 h-100 d-flex align-items-center justify-content-center p-5 text-muted"><i class="bi bi-people display-4 opacity-25"></i><p class="mt-2">No active staff shifts found.</p></div>
            <?php endif; ?>
        </div>
...
<script>
    function viewShiftDetails(shiftId) {
        // UPDATED LINK: Now uses print_shift.php
        document.getElementById('drillDownFrame').src = 'index.php?page=print_shift&shift_id=' + shiftId;
        new bootstrap.Modal(document.getElementById('drillDownModal')).show();
    }
    
    setInterval(() => { const now = new Date(); document.getElementById('liveClock').innerText = now.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}); }, 1000);
